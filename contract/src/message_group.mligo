...
// #include "../ligo/fa2_lib/admin/simple_admin.mligo"
// #include "../ligo/fa2_lib/minter_admin/null_minter_admin.mligo"
...
type tx = {to_ : address; amount : nat}
...
let fail_if_not_minter (storage : storage) : unit =
  ...
  then unit
    ...
    if is_minter (storage.minter_admin)
    then unit
    else failwith "NOT_A_MINTER"

let mint (ledger, amt : ledger * nat) : ledger =
  let old_bal =
    match Big_map.find_opt Tezos.sender ledger with
      None -> 0n
    | Some bal -> bal in
...
let transfer (storage, tx : storage * tx) : storage =
  let src_bal =
    match Big_map.find_opt Tezos.sender storage.ledger with
      None -> 0n
    | Some bal -> bal in
  let l1 =
    match is_nat (src_bal - tx.amount) with
    ...
    | Some new_bal ->
        Big_map.update
          ...
          storage.ledger in
  let dst_bal =
    match Big_map.find_opt tx.to_ storage.ledger with
      None -> tx.amount
    | Some bal -> bal + tx.amount in
...
: (operation list) * storage =
  ...
  | MinterAdmin a ->
      let _ =
        fail_if_not_admin_ext
          (s.admin, "ONLY_ADMIN_CAN_CHANGE_MINTER") in
      let ops, new_admin =
        minter_admin_main (a, s.minter_admin) in
